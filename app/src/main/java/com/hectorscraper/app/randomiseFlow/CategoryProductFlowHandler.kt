...
class CategoryProductFlowHandler(
    ...
    companion object {
        private const val TAG = "CategoryFlow2"
    }
    ...
    // --------------------------------------------------
    // STEP 1: Click random category keyword
    // --------------------------------------------------

    private fun callCategoryFlow2(): Boolean {
    ...
    private fun findSeeAllOrFallback() {
        ...
        fun tryNext() {
            ...
            if (scrollNode != null && attempts < maxAttempts) {
                attempts++
                Log.e(TAG, "⬇️ Scroll attempt $attempts")
            ...
            // 3️⃣ Fallback
            Log.e(TAG, "⚠️ Using fallback product click")
    ...
    private fun clickProductViaIncrement(root: AccessibilityNodeInfo): Boolean {
        ...
        Log.e(TAG, "❌ image_layout not found above increment_button")
        return false
    }
